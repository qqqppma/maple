...
if menu == "악마 길드원 정보 등록":
   ...
   # ✅ 본캐별 부캐 보기 (중복 제거 통합)
    ...
    if st.button("📤 등록하기", key="reg_submit"):
        ...
        st.info("업로드 파일명은 영어일 경우만 업로드가 가능합니다.")
    # --------------------------
    # ✏️ 기존 이벤트 수정 섹션
    # --------------------------
    st.markdown("---")
    ...
    if not event_list:
        ...
        display_names = [ev["title"] for ev in event_list]
        selected_name = st.selectbox("수정할 이벤트 선택", display_names, key="edit_selector")
        selected_event = next((ev for ev in event_list if ev["title"] == selected_name), None)
